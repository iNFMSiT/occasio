...
import { Upload, X, Camera, ArrowRight, Loader } from 'lucide-react';
...
function isHeic(file) {
  ...
  // iPhone files sometimes have an empty MIME type — check extension as fallback
...
export default function UploadStep() {
  const { images, settings, dispatch } = useWizard();
  ...
  const handleDrop = (e) => {
    ...
    const files = Array.from(e.dataTransfer.files).filter((f) => {
      ...
      // HEIC files from iPhones may have an empty MIME type
    ...
    <div className="animate-fade-in space-y-6">
      <div className="text-center space-y-2">
        ...
        <p className="text-text-muted">
          Add 1-2 clear photos. We'll use these to create personalized cards in wild styles.
        </p>
      </div>
          ...
          {images.map((img, i) => (
            <div key={i} className="relative group">
              <img
                src={img.preview}
                alt={img.name}
                className="w-36 h-48 object-cover rounded-lg border-2 border-surface-lighter"
              />
              <button
                onClick={(e) => {
                  e.stopPropagation();
