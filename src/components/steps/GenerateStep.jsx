...
export default function GenerateStep() {
  ...
  const [phase, setPhase] = useState('idle'); // idle, previewing, generating, done
  ...
  const needsPreview = cardCount > 2; // Skip preview for 1-2 cards
  const previewCount = Math.min(2, cardCount);
  const currentModel = GEMINI_CONFIG.models[settings.modelTier];

  // Get reference image base64 for face consistency
  const getReferenceBase64 = async () => {
    ...
    } catch { /* ignore - reference image is optional */ }
    return null;
  };
  ...
  const generateAll = async () => {
    ...
    try {
      ...
      if (needsPreview && previewCards.length > 0) {
        // We already have preview cards, generate the rest
          ...
          const results = await service.generateBatch(remainingItems, {
            ...
            onProgress: (p) => {
              dispatch({
                ...
                payload: { current: p.current + previewCount, total: cardCount },
              });
            },
          });
        ...
        // Generate all at once (for 1-2 cards, or if no preview)
  ...
  const handleCountChange = (count) => {
    ...
    // Reset blueprint and previews when count changes
  ...
  const handlePeekPrompts = () => {
    ...
    // Generate blueprint just to preview the prompts (don't dispatch yet)
    const bp = promptEngine.createBlueprint(anchorDescription, surveyData, selectedStyles, selectedThemes, cardCount);
    setPeekBlueprint(bp);
    setShowPrompts(true);
  };
  ...
  return (
    <div className="animate-fade-in space-y-6">
      ...
      {/* Card count selector — only show before generation starts */}
      {phase === 'idle' && (
      ...
      {/* Peek at prompts — before generation */}
      {phase === 'idle' && anchorDescription && (
        <div className="space-y-2">
          <div className="flex justify-center">
            <button
              onClick={handlePeekPrompts}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs text-text-muted hover:text-text border border-surface-lighter/50 hover:border-brand-light/30 transition-colors"
            >
              <Code size={12} />
            ...
            <div className="space-y-2 max-h-80 overflow-y-auto custom-scrollbar">
              {peekBlueprint.map((item, i) => (
                <PromptViewer
                  key={i}
                  prompt={item.prompt}
                  label={`Card #${i + 1} — ${item.style} · ${item.theme}`}
                  defaultOpen={i === 0}
                />
              ))}
            </div>
      ...
      {/* Prompts during/after generation */}
              ...
              <div key={i} className="relative group">
                {card.imageUrl ? (
                  <img
                    src={card.imageUrl}
                    alt={`Preview ${i + 1}`}
                    className="w-44 h-64 object-cover rounded-xl border-2 border-surface-lighter shadow-lg"
                  />
                ) : (
      ...
      <div className="flex justify-between items-center pt-2">
        ...
        <div className="flex gap-3">
          {phase === 'idle' && (
            <button
              onClick={needsPreview ? generatePreview : generateAll}
              disabled={isWorking}
          ...
          {previewCards.length > 0 && phase !== 'generating' && phase !== 'done' && (
            <>
              <button
                ...
                disabled={isWorking}
                className="flex items-center gap-1.5 px-4 py-2.5 text-sm border border-surface-lighter rounded-lg hover:bg-surface-light disabled:opacity-50 transition-colors"
              >
                <RefreshCw size={14} />
                Try Again
              </button>
              <button
                onClick={generateAll}
                disabled={isWorking}
                className="flex items-center gap-2 px-6 py-2.5 bg-brand hover:bg-brand-dark disabled:opacity-50 rounded-lg font-medium text-sm transition-colors animate-pulse-glow"
              >
                <Sparkles size={16} />
                Generate All {cardCount} Images
              </button>
            </>
          )}
