...
import {
  ...
  ArrowLeft, Package, Coffee, Smartphone, Frame, Code,
} from 'lucide-react';
...
function ProductMockup({ type, card }) {
  ...
  const mockupStyles = {
    cards: {
      container: 'bg-gradient-to-br from-gray-800 to-gray-900 rounded-2xl p-8 flex items-center justify-center',
      inner: 'relative',
      cardStyle: (i) => ({
    ...
    mug: {
      container: 'bg-gradient-to-br from-amber-900/30 to-stone-900/50 rounded-2xl p-8 flex items-center justify-center',
    },
    phone: {
      container: 'bg-gradient-to-br from-slate-800 to-slate-900 rounded-2xl p-8 flex items-center justify-center',
    },
    poster: {
      container: 'bg-gradient-to-br from-stone-800 to-stone-900 rounded-2xl p-8 flex items-center justify-center',
    },
  };

  if (type === 'cards') {
    return (
      <div className={mockupStyles.cards.container}>
  ...
  if (type === 'mug') {
    return (
      <div className={mockupStyles.mug.container}>
  ...
  if (type === 'phone') {
    return (
      <div className={mockupStyles.phone.container}>
  ...
  if (type === 'poster') {
    return (
      <div className={mockupStyles.poster.container}>
...
export default function GalleryStep() {
  const { cards, settings, dispatch } = useWizard();
  ...
  const regenerateCard = async (index) => {
    ...
    try {
      ...
      dispatch({
        type: 'UPDATE_CARD',
        payload: { index, card: { ...card, ...result } },
      });
    ...
    <div className="animate-fade-in space-y-6">
      <div className="text-center space-y-2">
        ...
        <p className="text-text-muted">
          {isSingle
            ? 'Preview your image on different products below.'
            : `${cards.length} images generated. Browse, regenerate, and preview on products.`}
        </p>
      </div>
      ...
      <div className="space-y-3">
            ...
            return (
              <button
                ...
                className={`flex items-center gap-1.5 px-3 py-2 rounded-lg text-sm transition-all ${
                  activeMockup === tab.id
                    ? 'bg-brand text-white'
                    : 'bg-surface-light text-text-muted hover:text-text border border-surface-lighter/30'
                }`}
              >
                <Icon size={14} />
                {tab.label}
        ...
        <div className="relative">
          ...
          {/* Coming soon badge */}
          <div className="absolute top-3 right-3 px-2 py-1 rounded-full bg-accent/90 text-black text-[10px] font-bold">
            ORDER COMING SOON
          </div>
          {/* Card selector dots */}
          <div className="flex gap-1.5 justify-center mt-3">
            {cards.map((_, i) => (
              <button
                key={i}
                onClick={() => setPreviewCard(i)}
                className={`w-2.5 h-2.5 rounded-full transition-all ${
                  i === previewCard ? 'bg-brand scale-125' : 'bg-surface-lighter hover:bg-brand-light/50'
                }`}
              />
            ))}
          </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
        {cards.map((card, i) => (
          <div key={i} className="space-y-1.5">
            <div
              className={`relative group rounded-xl overflow-hidden border-2 transition-all card-hover ${
                selected.has(i) ? 'border-brand shadow-lg shadow-brand/20' : 'border-surface-lighter'
              }`}
            >
              {card.imageUrl ? (
              ...
              <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                <button
                  onClick={(e) => { e.stopPropagation(); regenerateCard(i); }}
                  disabled={regenerating === i}
                  className="p-2.5 bg-white/20 rounded-full hover:bg-white/30 backdrop-blur-sm transition-colors"
                  title="Regenerate"
                >
                  <RefreshCw size={16} className={regenerating === i ? 'animate-spin' : ''} />
                </button>
                <button
                  onClick={(e) => { e.stopPropagation(); downloadCard(card, i); }}
                  className="p-2.5 bg-white/20 rounded-full hover:bg-white/30 backdrop-blur-sm transition-colors"
                  title="Download"
                >
                  <Download size={16} />
            ...
            <div className="flex justify-center">
              <RatingSlider
                value={card.rating ?? null}
                onChange={(val) => dispatch({
                  type: 'RATE_CARD',
                  payload: { index: i, rating: val },
                })}
                compact
              />
            </div>
            {/* Prompt viewer under each card */}
