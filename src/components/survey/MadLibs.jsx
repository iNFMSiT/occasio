...
export default function MadLibs({ value = [], onChange }) {
  const [templateIndex, setTemplateIndex] = useState(0);
  const [selections, setSelections] = useState({});

  const template = MAD_LIBS_TEMPLATES[templateIndex];

  const shuffle = () => {
    setTemplateIndex((i) => (i + 1) % MAD_LIBS_TEMPLATES.length);
    setSelections({});
  };
      ...
      <div className="bg-surface-light/50 rounded-xl p-5 space-y-4 border border-surface-lighter/30">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-medium">Template {templateIndex + 1}/{MAD_LIBS_TEMPLATES.length}</span>
          </div>
          <button
            onClick={shuffle}
